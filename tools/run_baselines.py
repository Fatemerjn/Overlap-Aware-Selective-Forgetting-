#!/usr/bin/env python3
"""
Run baseline method comparisons with a shared setup.
...
Example:
python tools/run_baselines.py --dataset cifar10 --seeds 0 1 --dry-run
"""
...
def validate_args(args: argparse.Namespace) -> None:
    ...
    invalid = [m for m in args.methods if m not in {"pall_original", "pall_modified", "ewc", "lwf", "er", "derpp"}]
    if invalid:
...
def run_commands(commands: List[List[str]], cwd: Path, dry_run: bool, continue_on_error: bool) -> int:
    ...
    for idx, cmd in enumerate(commands, start=1):
        ...
        seed = cmd[cmd.index("--seed") + 1] if "--seed" in cmd else "unknown"
...
def parse_args() -> argparse.Namespace:
    ...
    parser = argparse.ArgumentParser(description="Run baseline comparisons with shared args.")
    ...
    parser.add_argument("--methods", nargs="+", default=DEFAULT_METHODS, help="Method list to run.")
    parser.add_argument(
        "--forget-iters",
        type=int,
        default=50,
        help="forget_iters passed only to ER/DERPP methods (required for forgetting).",
    )
    ...
    parser.add_argument(
        "--commands-file",
        default=None,
        help="Path for command log file. Defaults to <output-root>/baseline_commands_<timestamp>.txt",
    )
...
def main() -> int:
    ...
    if args.commands_file is None:
        commands_file = output_root / f"baseline_commands_{timestamp}.txt"
    else:
