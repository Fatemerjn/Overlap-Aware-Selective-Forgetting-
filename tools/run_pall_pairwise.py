#!/usr/bin/env python3
"""
Run paired PALL comparisons with a fixed request schedule.

For each seed, this runner launches:
- pall_original
- pall_modified

Both methods receive the same --request_schedule_file for fair comparisons.

Example:
python tools/run_pall_pairwise.py \
...
def validate_args(args: argparse.Namespace) -> None:
    ...
    if args.protect_ratio is not None and not (0.0 <= args.protect_ratio <= 1.0):
        raise ValueError("protect_ratio must be in [0, 1].")
    if args.lambda_protect < 0.0:
        raise ValueError("lambda_protect must be >= 0.")
    if args.retrain_steps is not None and args.retrain_steps < 0:
...
def build_commands(args: argparse.Namespace, main_file: Path) -> List[List[str]]:
    ...
    for seed in args.seeds:
        for method in METHODS:
            ...
            # Modified-method knobs are only passed to pall_modified.
            if method == "pall_modified":
                if args.protect_ratio is not None:
                    cmd.extend(["--protect_ratio", str(args.protect_ratio)])
                cmd.extend(["--lambda_protect", str(args.lambda_protect)])
                if args.retrain_steps is not None:
                    cmd.extend(["--retrain_steps", str(args.retrain_steps)])
...
def parse_args() -> argparse.Namespace:
    ...
    parser = argparse.ArgumentParser(description="Run paired pall_original/pall_modified with a fixed schedule.")
    ...
    # Modified-method hyperparameters.
    parser.add_argument("--protect_ratio", type=float, default=0.2)
    parser.add_argument("--lambda_protect", type=float, default=0.1)
    parser.add_argument("--retrain_steps", type=int, default=50)
    ...
    parser.add_argument(
        "--commands-file",
        default=None,
        help="Path for command log file. Defaults to <output-root>/pall_pairwise_commands_<timestamp>.txt",
    )
...
def main() -> int:
    ...
    if args.commands_file is None:
        commands_file = output_root / f"pall_pairwise_commands_{timestamp}.txt"
    else:
