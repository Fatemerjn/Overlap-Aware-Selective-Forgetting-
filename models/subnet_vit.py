...
class maskedSequentialViT(nn.Sequential):
    def forward(self, *inputs):
        ...
        mask = inputs[1]
        mode = inputs[2]
        task = inputs[3] if len(inputs) > 3 else None
        for module in self._modules.values():
            if isinstance(module, SubnetEncoderBlock):
                x = module(x, mask, mode, task)
            else:
                x = module(x)
        return x
...
class SubnetMultiHeadAttention(nn.Module):
    ...
    def forward(self, Q, K, V, mask, mode='train', task=None):
        """
            inputs:
...
class SubnetEncoderBlock(nn.Module):
    ...
    def forward(self, input, mask, mode='train', task=None):
        torch._assert(input.dim() == 3, f"Expected (batch_size, seq_length, hidden_dim) got {input.shape}")
        x = self.ln_1(input)
        x, _ = self.mha(x, x, x, mask, mode, task)
        x = self.dropout(x)
        x = x + input
        y = self.ln_2(x)
...
class SubnetVisionTransformer(BaseModel):
    ...
    def _process_input(self, x, mask, mode, task=None):
        n, c, h, w = x.shape
        p = self.patch_size
    ...
    def forward(self, x, task=-1, mask=None, mode="train", returnt='out'):
        ...
        x = self._process_input(x, mask, mode, task)
        n = x.shape[0]
        ...
        x = self.ln(self.encoder_layers(self.d_out(x), mask, mode, task))
    ...
    def reinit_weights(self, combined_masks):
        ...
        for name, module in self.named_modules():
            if isinstance(module, SubnetLinear) or isinstance(module, SubnetConv2d) or isinstance(module, SubnetClassifier):
                if combined_masks != {}:
                    weight_mask = combined_masks[name + '.weight'].to(dtype=torch.bool, device=module.weight.device)
                    module.weight.data = torch.where(weight_mask, module.weight,
                                                     nn.init.kaiming_uniform_(torch.zeros_like(module.weight), a=math.sqrt(5)))
                    if module.bias is not None:
                        fan_in, _ = nn.init._calculate_fan_in_and_fan_out(module.weight)
                        bound = 1 / math.sqrt(fan_in)
                        bias_mask = combined_masks[name + '.bias'].to(dtype=torch.bool, device=module.bias.device)
                        module.bias.data = torch.where(bias_mask, module.bias,
