...
class SubnetBasicBlock(nn.Module):
    ...
    def forward(self, x, mask, mode='train', task=None):
...
class SubnetResNet(BaseModel):
    ...
    def forward(self, x, task=-1, mask=None, mode="train", returnt='out'):
        ...
        out = self.layer1(out, mask, mode, task)
        out = self.layer2(out, mask, mode, task)
        out = self.layer3(out, mask, mode, task)
        out = self.layer4(out, mask, mode, task)
