...
class RehearsalMemory(data.Dataset):
    ...
    def select_indices_by_random(self, targets):
        ...
        for curr_cls in np.unique(targets):
            ...
            chosen = random.sample(list(cls_ind), self.exemplars_per_class)
            result.extend(chosen)
            selected.update(chosen)
        remainder = self.exemplars_per_task - len(result)
        if remainder > 0:
            # If exemplars_per_task is not divisible by classes/task, fill the remainder uniformly.
            remaining = [idx for idx in range(len(targets)) if idx not in selected]
            if len(remaining) >= remainder:
                result.extend(random.sample(remaining, remainder))
            else:
                if remaining:
                    result.extend(remaining)
                if len(result) < self.exemplars_per_task:
                    result.extend(random.choices(result, k=self.exemplars_per_task - len(result)))
        return result
